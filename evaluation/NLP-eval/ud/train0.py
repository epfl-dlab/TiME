#evaluation/NLP-eval/ud/train0.py
import time
import tqdm
import wandb
import argparse
...
def main():
    ...
    parser.add_argument("--treebank_path",
                        default="/scratch/project_465001386/ud-treebanks-v2.13/")  # 2.15 for Albanian, Georgian!
    parser.add_argument("--version", type=str, default="2_0",
                        choices=('2_0', '1_2'))  # model trained on data of version
    ...
    train_loader = DataLoader(train_data, args.batch_size, shuffle=True, drop_last=True, num_workers=7,
                              collate_fn=CollateFunctor(train_data.pad_index))
    dev_loader = DataLoader(dev_data, args.batch_size, shuffle=False, drop_last=False, num_workers=7,
                            collate_fn=CollateFunctor(train_data.pad_index)) if dev_data is not None else None
    test_loader = DataLoader(test_data, args.batch_size, shuffle=False, drop_last=False, num_workers=7,
                             collate_fn=CollateFunctor(train_data.pad_index))
